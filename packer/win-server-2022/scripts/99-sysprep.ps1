# packer/win-server-2022/scripts/99-sysprep.ps1
#
# WHAT THIS IS
...
# genuinely annoying failure mode on a domain controller.
#
...
if (Test-Path $cbUnattend) {
    ...
    #
    # Found on the win11-client template, where it cost about two days and was
    # misdiagnosed twice (as the vTPM, then as automatic BitLocker). This script
    # carried the identical bug. The two Windows templates keep diverging by
    # having the same fault fixed in only one of them, so both are fixed here.
    #
...
Write-Host 'Sysprep complete. Packer will now stop the VM through the Proxmox API and convert it to template 9002.'
