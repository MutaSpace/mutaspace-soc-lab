...
$watchdogDir = 'C:\ProgramData\example'
...
Unregister-ScheduledTask -TaskName 'ExampleKeepAdminEnabled' -Confirm:$false -ErrorAction SilentlyContinue
...
if (Get-ScheduledTask -TaskName 'ExampleKeepAdminEnabled' -ErrorAction SilentlyContinue) {
...
try {
    ...
    if (Test-Path $profilePath) {
        ...
        $cmd = 'rmdir /s /q C:\Users\packer & schtasks /delete /tn ExampleRemovePackerProfile /f'
        $action    = New-ScheduledTaskAction -Execute 'cmd.exe' -Argument "/c $cmd"
        $trigger   = New-ScheduledTaskTrigger -AtStartup
        $principal = New-ScheduledTaskPrincipal -UserId 'SYSTEM' -LogonType ServiceAccount -RunLevel Highest
        Unregister-ScheduledTask -TaskName 'ExampleRemovePackerProfile' -Confirm:$false -ErrorAction SilentlyContinue
        Register-ScheduledTask -TaskName 'ExampleRemovePackerProfile' `
        ...
        Remove-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\RunOnce' `
            -Name 'ExampleRemovePackerProfile' -Force -ErrorAction SilentlyContinue
    } else {
...
# watchdog's task and C:\ProgramData\example both vanished mid-build, and a
# ExampleEnsureWinRM task registered from the user-data was gone after the clone's
# first reboot, with its log never written.
#
# A task registered HERE, before sysprep, survives into the image - proven by
# ExampleRemovePackerProfile above, which fires on a clone and deletes the stale
...
$log = 'C:\Windows\Temp\example-ensure-winrm.log'
...
while ((Get-Date) -lt $deadline) {
    ...
    if (-not (Get-NetFirewallRule -Name 'Example-WinRM-HTTP' -ErrorAction SilentlyContinue)) {
        New-NetFirewallRule -Name 'Example-WinRM-HTTP' -DisplayName 'WinRM HTTP (Example lab)' `
    ...
    if ($listening) {
        ...
        Unregister-ScheduledTask -TaskName 'ExampleEnsureWinRM' -Confirm:$false -ErrorAction SilentlyContinue
...
Unregister-ScheduledTask -TaskName 'ExampleEnsureWinRM' -Confirm:$false -ErrorAction SilentlyContinue
Register-ScheduledTask -TaskName 'ExampleEnsureWinRM' -Action $eAct -Trigger $eTrg -Principal $ePrn | Out-Null
if (Get-ScheduledTask -TaskName 'ExampleEnsureWinRM' -ErrorAction SilentlyContinue) {
    Write-Host 'ExampleEnsureWinRM registered (AtStartup, dormant until a clone boots).'
} else {
    throw 'Failed to register ExampleEnsureWinRM - clones would come up unreachable.'
}
