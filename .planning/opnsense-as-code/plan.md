...
- **config.xml seed** (`packer/opnsense-267/config/config.xml.pkrtpl.hcl`):
  ...
  - `<IDS>` already templated at 485-512 (`suricata_enabled` default true,
    ...
    AND a Wazuh-side syslog listener (Wave 4). [CORRECTION 2026-07-24: the assumption that Wazuh's
    Suricata decoders are built-in is FALSE for the SYSLOG path — see W4.4 below; a custom decoder
    was required and is now shipped.]
...
| 4.4 | **EVE→Wazuh, Wazuh half** (beside 70-detections; depends on 40-wazuh-server): add a `<remote><connection>syslog</connection><port>514</port><protocol>udp</protocol><allowed-ips>10.10.10.1</allowed-ips></remote>` listener to ossec.conf on wazuh-01. **Deviation flagged:** a NEW `<remote>` listener requires a full MANAGER RESTART, not the reload-only pattern 70-detections uses — sequence the restart in a maintenance moment so agents aren't dropped mid-class (agents buffer and reconnect, but do it deliberately). **[CORRECTED 2026-07-24 — DONE & PROVEN]** The assumption "no custom decoder needed" was WRONG: Wazuh ships the Suricata *rules* (0475-suricata_rules.xml, 86xxx) but NO decoder for the syslog path — the built-in `json` decoder only matches logs beginning with `{`, which the syslog frame `suricata[pid]: {json}` defeats (`wazuh-logtest` → "No decoder matched"). A custom decoder (`files/wazuh-decoders/suricata-eve.xml`: `<decoder name="json">` scoped by `program_name suricata`, running `JSON_Decoder`, named `json` so `decoded_as json` fires rule 86600→86601) is now deployed by 70-detections.yml via the API and reloaded with analysisd (no restart needed for a decoder-only change; the restart is still needed once for the new listener socket). | `ansible/playbooks/70-detections.yml`, `ansible/files/wazuh-decoders/suricata-eve.xml` | **PASSED:** `curl http://testmynids.org/uid/index.html` from ubuntu-app-01 across fw-01 → Wazuh rule 86601 (sid 2100498) in alerts.json. |
...
| # | Task | Files | Verify |
|---|------|-------|--------|
| 5.1 | **DONE (2026-07-24).** Taskfile `fw:*` namespace complete: `fw:preflight` (W1), `fw:rebuild-template` (W2), `fw:config` (full 05-fw-config apply), `fw:egress-open`/`fw:egress-close` (the `-e fw_egress_state` toggle), `fw:suricata` (both halves + wazuh restart), `fw:status` (read-only API health curl). Verbs run `ansible-playbook`/`curl` directly on the control node (the jumpbox). | `Taskfile.yml` | **VERIFIED:** Taskfile parses (7 fw verbs); `fw:status` logic returns "OPNsense 26.7" + "IDS status: running"; `fw:egress-open`→rule enabled=1, `fw:egress-close`→enabled=0; `fw:config` idempotent (ok=6 changed=1). NB: `task` binary is not installed on the workstation/jumpbox, so verbs were validated via the underlying commands, not `task --list`. |
...
6. **EVE→Wazuh is two objects + a listener:** ids_general syslog toggle (inert alone) + an
   ...
   deliberately). [CORRECTED 2026-07-24: the 86xxx are RULES; the syslog path needs a custom
   decoder — `files/wazuh-decoders/suricata-eve.xml` — because the built-in json decoder won't
   match syslog-framed JSON. Now shipped by 70-detections.yml.]
